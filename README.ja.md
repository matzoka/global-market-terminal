...
- **CSVエクスポート** — 「注目市場」パネルに現在表示中の一覧（区分・銘柄コード・表示記号・名称・価格・前日比・変化基準・出所・配信区分・状態・基準時刻・取得時刻）を、Excel等で扱いやすい UTF-8 (BOM付き) のCSVとして保存します。
- **最終更新時刻** — 「注目市場」パネル見出しに、表示中データの最終取得時刻を1か所だけ表示します。各市場行へ同じ時刻を並べません。銘柄ごとの基準時刻はバッジのツールチップ・詳細ドロワー・CSVで確認できます。
...
│   │   ├── csv.js          # CSV生成・ダウンロード補助（一覧のエクスポート）
